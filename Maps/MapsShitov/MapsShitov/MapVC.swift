...
class MapVC: UIViewController , CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        if let location = locations.last?.coordinate { mapToCoordinate(coordinate:  location)}
    }
    ...
    func pointsPositisionalCollege(){
        ...
        for number in 0..<arrayLat.count { pointsArray.append(Points(lat: arrayLat[number], lon: arrayLon[number], name: arrayName[number]))}
    }
    
    func pointsPositions () {
        for number in 0..<pointsArray.count { let point = MKPointAnnotation()
